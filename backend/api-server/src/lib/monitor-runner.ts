    ...
    if (lowScorePages.length > 0) {
      ...
      for (const page of lowScorePages.slice(0, 3)) { // Alert on up to 3 pages per run
        const alertHtml = renderLowScoreAlertEmail({
          ...
          score: page.currentScore!,
          previousScore: page.previousScore,
          appUrl,
        });
        await sendEmail({
          to: site.email,
          subject: `⚠️ ${site.domain} — Page score dropped to ${page.currentScore}/100`,
          html: alertHtml,
        });
      }
      log?.info({ siteId: site.id, count: lowScorePages.length }, "low score alerts sent");
    }
  }
