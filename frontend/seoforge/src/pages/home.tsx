...
export default function Home() {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 bg-white text-slate-900 overflow-hidden">
        {/* HERO */}
          ...
          <div className="mx-auto grid max-w-7xl gap-14 px-6 py-24 lg:grid-cols-2 lg:items-center lg:py-32">
            <div>
              ...
              <div className="mt-10 rounded-3xl border border-slate-200 bg-white p-4 shadow-2xl shadow-slate-200/60">
                <div className="flex flex-col gap-4 md:flex-row">
                  <input
                    ...
                    className="h-12 flex-1 rounded-2xl border border-slate-200 px-5 text-sm outline-none transition focus:border-blue-500"
                  />

                  <Link href={workspaceHref}>
                    <button className="h-12 rounded-2xl bg-blue-600 px-8 text-sm font-bold text-white shadow-lg shadow-blue-500/30 transition hover:bg-blue-700">
                      Analyze Free
                    </button>
                  </Link>
                </div>
              ...
              <div className="mt-10 flex flex-wrap gap-8 text-sm font-semibold text-slate-500">
                <div>
                  <span className="text-xl font-black text-slate-900">50K+</span>
                  <div>Pages Audited</div>
                </div>
                <div>
                  <span className="text-xl font-black text-slate-900">+47</span>
                  <div>Average SEO Score Increase</div>
                </div>
                <div>
                  <span className="text-xl font-black text-slate-900">Instant</span>
            ...
            <div className="mt-16 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
              {features.map((feature) => (
                ...
                >
                  ...
                  <p className="mt-4 leading-7 text-slate-600 text-sm">{feature.desc}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="mt-20 grid gap-8 md:grid-cols-3">
              {howItWorks.map((item) => (
                <div key={item.step} className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-sm">
                  <div className="text-5xl font-black text-blue-100">{item.step}</div>
                  <h3 className="mt-6 text-xl font-black">{item.title}</h3>
                  <p className="mt-4 leading-7 text-slate-600 text-sm">{item.desc}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="mx-auto max-w-7xl overflow-hidden rounded-[40px] bg-gradient-to-r from-slate-950 to-blue-950 px-10 py-20 text-white shadow-2xl">
            <div className="max-w-3xl">
              ...
              <div className="mt-10 flex flex-wrap gap-4">
                <Link href={workspaceHref}>
                  <button className="rounded-2xl bg-white px-8 py-4 text-sm font-bold text-slate-900 transition hover:bg-slate-100">
                    Run Free Audit
                  </button>
                </Link>

                <Link href="/pricing">
                  <button className="rounded-2xl border border-white/20 bg-white/10 px-8 py-4 text-sm font-bold text-white backdrop-blur-xl transition hover:bg-white/20">
                    View Demo
                  </button>
                </Link>
              </div>
