...
import { customFetch, useOptimizeHtml, useGenerateBlogArticle, useGetAgencySettings } from "@workspace/api-client-react";
import { CheckCircle2, UploadCloud, Copy, RefreshCw, Download, Play, AlertCircle, FileCode, Globe, CheckSquare, FileDown, FileText, Sparkles, X, Languages, MapPin, Users, Volume2, TrendingUp, ArrowRight, Bot, ThumbsDown, ThumbsUp, FileText as FileTextIcon } from "lucide-react";
...
export function SinglePageOptimizer() {
  ...
  const handleExportPdf = async () => {
    ...
    const doc = new jsPDF();
    const pageTitle = extractMeta(optimizeMutation.data.optimizedHtml).title;
    
    // Header with branding
    doc.setFontSize(20);
    doc.setTextColor(37, 99, 235);
    doc.text(agencySettings?.brandName || "SEOaxe", 14, 22);
    
    doc.setFontSize(12);
    doc.setTextColor(100, 100, 100);
    doc.text(agencySettings?.tagline || "SEO Repair Engine for Existing Website Pages", 14, 32);
    ...
    doc.text("Page Audit Receipt", 14, 50);
    
    // Summary info
    doc.setFontSize(11);
    doc.setTextColor(60, 60, 60);
    doc.text(`Page: ${pageTitle || "Untitled Page"}`, 14, 60);
    doc.text(`Date: ${new Date().toLocaleDateString()}`, 14, 68);
    doc.text(`Repairs Applied: ${optimizeMutation.data.changes.length}`, 14, 76);
    
    // Scores
    doc.setFontSize(12);
    doc.setTextColor(0, 0, 0);
    doc.text("Score Improvement", 14, 90);
    
    const tableData = [
      ["Metric", "Before", "After", "Improvement"],
      ["Overall", `${optimizeMutation.data.originalScore.overall}`, `${optimizeMutation.data.score.overall}`, `+${optimizeMutation.data.scoreImprovement.overall}`],
      ["Technical SEO", `${optimizeMutation.data.originalScore.technical}`, `${optimizeMutation.data.score.technical}`, `+${optimizeMutation.data.scoreImprovement.technical}`],
      ["Content SEO", `${optimizeMutation.data.originalScore.content}`, `${optimizeMutation.data.score.content}`, `+${optimizeMutation.data.scoreImprovement.content}`],
      ["AI Answer Readiness", `${optimizeMutation.data.originalScore.aeo}`, `${optimizeMutation.data.score.aeo}`, `+${optimizeMutation.data.scoreImprovement.aeo}`],
    ];
    ...
    doc.setFontSize(12);
    doc.setTextColor(0, 0, 0);
    doc.text("Changes Applied", 14, finalY);
    
    const changesTableData = optimizeMutation.data.changes.map((change, idx) => [(idx + 1).toString(), change]);
    autoTable(doc, {
      startY: finalY + 8,
      head: [["#", "Change"]],
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(`Generated by ${agencySettings?.brandName || "SEOaxe"}`, 14, doc.internal.pageSize.height - 10);
      doc.text(`Page ${i} of ${pageCount}`, doc.internal.pageSize.width - 40, doc.internal.pageSize.height - 10);
    }
    
    doc.save(`${pageTitle.replace(/[^a-z0-9]/gi, "_") || "audit"}_receipt.pdf`);
    toast({ title: "PDF exported", description: "Audit receipt downloaded successfully!" });
  };
        ...
        <div className="space-y-8">
          <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                >
                  <FileTextIcon className="h-4 w-4 mr-2" /> Export PDF
                </Button>
                <Button
                  variant="outline"
                  onClick={() => {
              ...
              <AccordionContent>
                ...
                <div className="grid gap-6 md:grid-cols-2">
                  ...
                  <div>
                    ...
                    {(optimizeMutation.data.aiReview?.flags?.length ?? 0) > 0 ? (
                      <ul className="space-y-2">
