...
SUCCESSFUL=0
FAILED=0
RESULTS='{"total":100,"successful":0,"failed":0,"results":[]}'

for i in "${!KEYWORDS[@]}"; do
  INDEX=$((i + 1))
  KEYWORD="${KEYWORDS[$i]}"
  
  echo "Generating article $INDEX/100: $KEYWORD"
  
  RESPONSE=$(curl -s -X POST "$API_URL/api/blog-from-keyword" \
    -H "Content-Type: application/json" \
    -d "{\"keyword\":\"$KEYWORD\",\"tone\":\"authoritative\"}")
  
  if echo "$RESPONSE" | jq -e '.title' > /dev/null 2>&1; then
    TITLE=$(echo "$RESPONSE" | jq -r '.title')
    META_TITLE=$(echo "$RESPONSE" | jq -r '.metaTitle')
    META_DESC=$(echo "$RESPONSE" | jq -r '.metaDescription')
    SLUG=$(echo "$RESPONSE" | jq -r '.slug')
    CONTENT=$(echo "$RESPONSE" | jq -r '.content')
    
    FILENAME=$(printf "%03d-%s.html" "$INDEX" "$SLUG")
...
echo "Failed: $FAILED"
echo "Articles saved to: $OUTPUT_DIR"
