...
async function generateArticle(keyword: string, index: number): Promise<BlogPost | null> {
  try {
    const response = await fetch(`${API_URL}/api/blog-from-keyword`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        ...(API_KEY ? { Authorization: `Bearer ${API_KEY}` } : {}),
      },
      body: JSON.stringify({ keyword, tone: "authoritative" }),
    });

    if (!response.ok) {
      console.error(`Failed to generate article ${index + 1}: ${keyword}`, response.status);
      return null;
    }

    const data = await response.json();
    console.log(`Generated article ${index + 1}/${keywords.length}: ${data.title}`);
    return data;
  } catch (error) {
    console.error(`Error generating article ${index + 1}: ${keyword}`, error);
    return null;
  }
}
...
async function main() {
  ...
  const results: { keyword: string; success: boolean; file?: string }[] = [];

  for (let i = 0; i < keywords.length; i++) {
    const keyword = keywords[i];
    const article = await generateArticle(keyword, i);

    if (article) {
      const filename = `${String(i + 1).padStart(3, "0")}-${article.slug}.html`;
...
${article.content}
      ...
      results.push({ keyword, success: true, file: filename });
      
      // Small delay to avoid rate limiting
      await new Promise((resolve) => setTimeout(resolve, 1000));
    } else {
      results.push({ keyword, success: false });
    }
  }
